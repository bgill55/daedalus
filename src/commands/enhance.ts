 ...
 */
function buildEnhanceContext(ctx?: CommandContext): string {
  if (!ctx) return '';
  const parts: string[] = [];
  ...
  const firstFile = ctx.activeFiles?.keys().next().value as string | undefined;
  if (firstFile) startDir = path.dirname(firstFile);

  let root = startDir;
  const rootMarkers = ['package.json', 'AGENTS.md', 'CLAUDE.md', '.git'];
  for (let dir = startDir; dir !== path.dirname(dir); dir = path.dirname(dir)) {
    if (rootMarkers.some(m => fs.existsSync(path.join(dir, m)))) { root = dir; break; }
  }
  ...
  try {
    ...
    if (fs.existsSync(pkgPath)) {
      const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8')) as {
        name?: string; dependencies?: Record<string, string>; devDependencies?: Record<string, string>; scripts?: Record<string, string>;
      };
      ...
      for (const dep of Object.keys(deps)) {
        if (markers[dep]) stackHints.push(markers[dep]);
      }
      const scripts = pkg.scripts ? Object.keys(pkg.scripts) : [];
      parts.push(`PROJECT: ${name} (${path.basename(root)})`);
      if (stackHints.length) parts.push(`DETECTED STACK: ${[...new Set(stackHints)].join(', ')}`);
  ...
  if (conventionFiles.length) parts.push(`CONVENTION FILES PRESENT: ${conventionFiles.join(', ')}`);
