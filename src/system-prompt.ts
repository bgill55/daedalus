...
- **Action Plans vs. Informational Queries**: Only propose an execution plan when the user requests an actual code modification or multi-step implementation task. When the user asks informational, search, discovery, or instructional questions (e.g. "what are the steps to test this?", "how do I run this?", "explain how this module works", "find trigger words in config"), answer the question or inspect directly.
- **Read-Only Tools NEVER Require A Plan**: Inspecting code using read-only tools (\`read_file\`, \`search_files\`, \`list_files\`, \`find_symbol\`, \`get_definition\`, \`get_references\`, \`git_status\`, \`git_diff\`) must NEVER trigger a proposed plan, a plan header, or an approval request. Execute read-only tools IMMEDIATELY on the same turn. Never ask permission to read or search files.
- **NEVER Output Pseudo-Tool Calls in Prose**: Do NOT emit tool calls inside Markdown code blocks (e.g. \`\`\`python read_file(...)\`\`\` or \`search_files(...)\`) or as plain text. If you intend to call a tool, invoke it through the native tool execution mechanism.
