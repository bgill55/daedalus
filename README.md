...
<p align="center">
  ...
  <a href="https://discord.gg/74pCA68KGK"><img src="https://img.shields.io/badge/Discord-Join%20Community-5865F2?logo=discord&logoColor=white" alt="Discord Community" /></a>
...
Read the design thesis in the [Daedalus Manifesto](docs/manifesto.md) and the architectural foundations in the [B.R.A.G. & Marathon Whitepaper](docs/whitepaper-brag-marathon.md) — grounded output, honest failure, and a CLI that will absolutely roast your type errors on the way to fixing them.
...
- **Companion Web UI & Sovereign Mobile PWA Companion (`/webui`)** — native local HTTP, WebSocket & Server-Sent Events telemetry dashboard (`http://localhost:3888`) featuring a sleek Greek mythic dark-mode interface, real-time token streaming, Chronicles session archives, active model switcher modal, in-chat tool accordions, hover code copy, and multimodal paste/drag-and-drop. Fully installable as a mobile-first **Progressive Web App (PWA)** with offline service worker caching (`sw.js`), 48px tactile touch targets, **QR Code Remote Pairing (`/api/qr`)** over LAN/Tailscale, and real-time **Milestone Push Notifications**.
...
### Multi-Agent, Orchestration & Autonomous Marathon

| Command | Description |
|---------|-------------|
| `/orchestrate <goal>` / `/orc` / `/run` / `/o` | Orchestrate agents for a goal |
| `/marathon <goal> \| status \| resume \| pr \| rollback \| abort` | Multi-day autonomous software development (Harness-of-Harness meta-loop) |
| `/autopilot <feature>` | Autonomously implement a feature: branch, code, test, commit, and PR |
| `/hunt <bug description>` / `/bug` | Autonomously hunt down and fix a bug: reproduce, locate root cause, fix, verify |
...
### Developer Tools, Git, MCP & Web UI
